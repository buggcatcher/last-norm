...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/24 17:54:19 by mailinci          #+#    #+#             */
/*   Updated: 2025/07/24 17:54:20 by mailinci         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token	*ft_word(t_shell_state *state, t_token **new, char **input)
{
	...
	if (var == 1)
		...
		result = ft_create_token_dollar(TK_DOLLAR_8, buffer, \
										ft_strlen_v(buffer), start);
	free(buffer);
	*new = result;
	return (result);
}
